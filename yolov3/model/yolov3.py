...
class YOLOv3(nn.Module):
    def __init__(self, device, num_classes=20, anchor_size=None, trainable=False, conf_thresh=0.01, nms_thresh=0.5,
                 topk=100, model_name='darknet53', pretrained=True):
        super().__init__()
        ...
        self.num_levels = 3  # L
        self.num_anchors = len(anchor_size) // self.num_levels  # A
        self.anchor_size = torch.as_tensor(anchor_size).float().view(self.num_levels, self.num_anchors, 2)  # [L, A, 2]
        ...
        ## neck network: SPP
        self.neck = build_neck(in_channels=feat_dim[-1], out_channels=feat_dim[-1])
