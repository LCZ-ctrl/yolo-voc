...
from config import anchor_size, iou_thresh, anchor_thresh
...
class Criterion(object):
    def __init__(self, device, num_classes=20, obj_w=1.0, cls_w=1.0, box_w=5.0):
        ...
        # matcher
        self.matcher = Yolov5Matcher(num_classes, 3, anchor_size, anchor_thresh, iou_thresh)
    ...
    def __call__(self, outputs, targets, epoch=0):
        ...
        pred_obj = torch.cat(outputs['pred_obj'], dim=1).view(-1)  # [B*M,]
        pred_cls = torch.cat(outputs['pred_cls'], dim=1).view(-1, self.num_classes)  # [B*M, C]
        pred_box = torch.cat(outputs['pred_box'], dim=1).view(-1, 4)  # [B*M, 4]

        # ------------------ Label Assignment ------------------
        ## match predicted grids with ground truth boxes
        gt_objectness, gt_classes, gt_bboxes = self.matcher(fmp_sizes=fmp_sizes, fpn_strides=fpn_strides,
                                                            targets=targets)
        ...
        gt_classes_pos = gt_classes[pos_masks] * ious.detach().unsqueeze(-1).clamp(min=0.)
